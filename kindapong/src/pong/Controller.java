...
public class Controller implements EventHandler<KeyEvent> {
    ...
    private void bounce(Rectangle rect){
        ...
        //if rect is the paddle change ball x velocity based upon its distance from the center of the paddle
        if (rect instanceof Paddle) {
            this.ball.setVelocityX(((Math.abs(ballCenterX - (rectLeft + 50)) / 10)+1)  * (this.ball.getVelocityX() / Math.abs(this.ball.getVelocityX())));
        }
        //cap velocity
        if (ballVelocityX > 9){
        ...
        //Checks to see if the ball hit the corner by checking which quartile of the ball struck the rect
        if ((ballCenterX < rectLeft || ballCenterX > rectRight)){
            if ((ballCenterY < rectTop && ballVelocityY > 0 && ballCenterX < rectLeft && ballVelocityX > 0) ||
                    (ballCenterY > rectBottom && ballVelocityY < 0 && ballCenterX < rectLeft && ballVelocityX > 0) ||
                    (ballCenterY < rectTop && ballCenterX > rectRight && ballVelocityY > 0 && ballVelocityX < 0) ||
                    (ballCenterY < rectBottom && ballCenterX < rectRight && ballCenterY < 0 && ballVelocityX < 0)){
            ...
            //if in between top and bottom of rectangle switch X velocity because it is hitting the side
    ...
    private void updateAnimation() {
        ...
        if (ball.getBoundsInParent().intersects(this.bottomWall.getBoundsInParent())){
            this.lives--;
            this.reset();
        }
        this.ball.step();
    ...
    private void startScreen(){
        this.startLabel.setText(String.format("To complete the level, destroy all " +
                "bricks without losing 3\n lives. Use the arrow keys or A and D to " +
                "move\n left and right. The closer the ball strikes to the\n edges of " +
                "the paddle the faster it will bounce.\n Press the Start button begin"));
        this.startLabel.setTextFill(Color.WHITE);
        this.startLabel.setBackground(new Background(new BackgroundFill(Color.BLACK, CornerRadii.EMPTY, Insets.EMPTY)));
        this.startOfGame = true;
    }

    public void startNewLevel(int rows , int columns , List<Boolean[]> list){
        lives = 3;
        this.livesLeft.setText(String.format("Lives left: %d", this.lives));
